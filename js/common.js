...
//运动封装函数
function startMove(obj, josn) {
	clearInterval(obj.tim);
	obj.tim = setInterval(function() {
		var flag = true;
		for(var arrt in josn) {
			if(arrt == "opacity") {
				var icur = Math.round(parseFloat(getstyle(obj, arrt)) * 100);
			} else {
				var icur = parseInt(getstyle(obj, arrt));
				//				var iCur = parseInt(getStyle(obj, attr));

			}
			var target = josn[arrt];
